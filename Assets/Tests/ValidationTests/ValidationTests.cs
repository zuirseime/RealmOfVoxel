...
public class ValidationTests
{
    ...
    [UnityTest] // FR4.01 + FR4.02 + FR4.03 + FR7.01
    public IEnumerator TSS_06_EnemyChangesStates()
    {
        Queue<System.Type> states = new();
        states.Enqueue(typeof(DummyEnemyWanderState));
        states.Enqueue(typeof(DummyEnemyChaseState));
        states.Enqueue(typeof(DummyEnemyAttackState));

        EntityState enemyState = TestHelper.GetPrivateFieldValue(testEnemy, "_currentState") as EntityState;
        Assert.IsInstanceOf(states.Dequeue(), enemyState, $"Enemy should have Wander state.");

        HealthBar healthBar = pauseMenuManager.GetComponentInChildren<HealthBar>();

        float initialHealth = healthBar.Slider.value;
        bool playerDamaged = false;

        player.HealthChanged += (sender, args) =>
        {
            playerDamaged = true;
            Assert.Less(healthBar.Slider.value, initialHealth, "Player health should decrease after enemy attack.");
        };
        testEnemy.StateChanged += (sender, args) =>
        {
            var state = states.Dequeue();
            Assert.IsInstanceOf(state, args, $"Enemy should change to {state.Name} state.");
            if (states.Count == 0)
                testEnemy.StateChanged -= (sender, args) => { };
        };

        player.SetDestination(testEnemy.transform.position);

        float timeout = 10f;
        float startTime = Time.time;

        while ((!playerDamaged || states.Count > 0) && Time.time < startTime + timeout)
        {
            yield return null;
        }
    }
